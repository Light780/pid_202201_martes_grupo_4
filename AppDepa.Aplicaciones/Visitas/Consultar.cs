        ...
        public class Handler : IRequestHandler<ListarVisitas, List<VisitaDto>>
        {
            ...
            public async Task<List<VisitaDto>> Handle(ListarVisitas request, CancellationToken cancellationToken)
            {
                var query = from v in context.Visita
                            ...
                            //where (v.VisitaId == 0)
                            where (request.NombreCompleto == "" || request.NombreCompleto == null || p.NombreCompleto.Contains(request.NombreCompleto))
                            ...
                            select new VisitaDto
                            {
                                ...
                                NombrePersona = p.NombreCompleto,
                                TipoDoc = utils.BuscarParametro(p.TipoDocumentoId, "TIPO_DOCUMENTO_PERSONA"),
                                Documento = p.Documento,
                                NroDepartamento = p.Departamento.NroDepartamento,
