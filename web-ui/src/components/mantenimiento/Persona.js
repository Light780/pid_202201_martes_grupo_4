...
function Persona() {
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, listaPersona.length - page * rowsPerPage);
  const peticionGet = () => {
  ...
  const peticionDelete = (e) => {
    ...
    borrarPersona(persona.personaId).then((respuesta) => {
      ...
      if (respuesta.status === 200) {
        mensaje =
          "Persona " +
          (persona.eliminado ? "activada" : "eliminada") +
          " correctamente";
        dispatch({
          type: "OPEN_SNACKBAR",
          openMensaje: {
  ...
  const validarForm = (persona) => {
    ...
    if (persona.telefono === "") {
      ...
      persona.telefono.trim().length !== 9 &&
      persona.telefono.trim().length !== 7
    ) {
        ...
        <form className={styles.modalForm}>
          <Grid container spacing={2} justifyContent="center">
            ...
            <Grid item xs={12} md={6}>
              <TextField
                name="documento"
                error={Boolean(errores?.documento)}
            ...
            <Grid item xs={12} md={6}>
              <SelectParametro
                concepto="TIPO_DOCUMENTO_PERSONA"
                error={Boolean(errores?.tipoDocumentoId)}
                errorMessage={errores?.tipoDocumentoId}
            ...
            <Grid item xs={12} md={6}>
              <SelectParametro
                concepto="TIPO_PERSONA_ID"
                error={Boolean(errores?.tipoPersonaId)}
                errorMessage={errores?.tipoPersonaId}
                name="tipoPersonaId"
                className={styles.inputMaterial}
      ...
      <Container component="main" maxWidth="md" justifyContentContent="center">
        ...
        <form className={styles.modalForm}>
          <Grid container spacing={2} justifyContent="center">
            <Grid item xs={12} md={12}>
              <TextField
                error={Boolean(errores?.nroPersona)}
                helperText={errores?.nroPersona}
                name="nombreCompleto"
                className={styles.inputMaterial}
                label="Nombre Completo"
                onChange={handleChange}
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                label="Documento"
                onChange={handleChange}
                value={persona && persona.documento}
              ></TextField>
            </Grid>
            <Grid item xs={12} md={6}>
              <SelectParametro
            ...
            <Grid item xs={12} md={6}>
              <SelectDepartamento
                value={persona && persona.departamentoId}
                error={Boolean(errores?.departamentoId)}
                errorMessage={errores?.departamentoId}
                name="departamentoId"
                className={styles.inputMaterial}
  ...
  const bodyEliminar = (
    <div className={styles.modal}>
      <Container component="main" maxWidth="md" justifyContent="center">
        ...
        >
          <b>
            {persona.documento} - {persona.nombreCompleto}
          </b>
        </Typography>
        <Grid container spacing={2} justifyContent="center">
          <Grid item xs={6} md={6}>
            ...
            <Paper className={styles.paperBody}>
              <Grid container spacing={2} justifyContent="flex-start">
                ...
                <Grid item container xs={3} md={2}>
                  ...
                  <Grid item xs={2} md={2}>
                    <Checkbox
                      checked={checkFiltro.filtroTipoPersonaId}
                      className={styles.inputMaterial}
                      style={style.checkFiltro}
                      onChange={handleCheckFiltro}
                      color="primary"
                      value={checkFiltro.filtroTipoPersonaId}
                      name="filtroTipoPersonaId"
                    />
                  </Grid>
                </Grid>
                      ...
                      .map((persona, index) => (
                        ...
                        >
                          ...
                          <TableCell size="small" align="center">
                            ...
                            <IconButton
                              color={
                                persona.eliminado ? "success" : "secondary"
                              }
                              component="span"
                              size="medium"
                              onClick={() => {
